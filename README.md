...
仓库内的 [`.github/workflows/deploy-server.yml`](.github/workflows/deploy-server.yml) 会在 `main` 分支的 AMD64、ARM64 Linux 后端包或 `updates/` 资源发生变化时自动发布，也可以在 GitHub Actions 页面手动运行。工作流通过 SSH 登录服务器，根据 `uname -m` 自动选择后端程序（`x86_64` 使用 AMD64，`aarch64`/`arm64` 使用 ARM64），在 `/data/early-sleep-family-app` 执行 `git pull --ff-only origin main`，然后重启监听 `31080` 端口的 API 并检查 `/ping`。新版本启动失败时，会尝试启动同架构的回退程序；首次部署会用部署前仓库中的程序初始化回退副本。
...
前端按日常任务分为「今天 / 早睡 / 菜价 / 家庭」四个入口：

- 今天：保留原版月亮打卡卡片及最近一次记录，将双人分数、双方统计和图标周历整合为紧凑的“一起积攒好梦”概览；一天可多次记录当前时间，以最后一次为准。打卡卡片下仅显示待自己确认的申请，没有时隐藏。
- 早睡：周历选日、补卡与豁免、待确认申请，以及本周和历史周报。
- 菜价：独立二级录入页，一张表单填写商品、店铺和价格，可选信息折叠；同一会话内自动保留未保存草稿，支持保存后连续录入。
- 家庭：成员、积分和奖励规则、30 天复盘、完整备份与服务器设置。

二级页面支持浏览器返回；打卡编辑失败保留输入，规则编辑不被普通定时刷新覆盖。完整设计见 [前端交互重构](docs/前端交互重构.md)。

本地浏览器回归脚本为 `mobile/scripts/verify-interactions.mjs`，需要可用的 Playwright 和 Chrome；先启动使用临时 `DATA_DIR` 的后端和前端开发服务，再运行：

```bash
UX_API_URL=http://127.0.0.1:18080 UX_WEB_URL=http://127.0.0.1:5174 node mobile/scripts/verify-interactions.mjs
```

如果 Playwright 由外部运行时提供，设置 `PLAYWRIGHT_MODULE` 为其模块路径。脚本仅允许连接本机服务，创建独立测试家庭，输出截图和测试结果到临时目录（可用 `UX_OUTPUT_DIR` 指定）。Android 实机返回键与软键盘仍需在打包后验证。
...
正式服默认地址为 `http://47.108.215.105:31080`。已有安装保留原地址，可在“设置 → 更换服务器地址”或连接失败页直接修改，无需退出家庭；本机备份和旧快照会保留。Android 1.3.1（versionCode 7）增加系统文件选择器导出 JSON；使用相同包名、原签名的 APK 覆盖安装可保留 App 数据，不要先卸载。Web 1.3.5 要求此完整安装包，旧安装包需先升级。

从 Web 1.3.5 开始，登录后的家庭刷新及菜价操作会自动请求完整导出，并按服务器地址、家庭 ID 保存到本机 `localStorage`。备份包含打卡、规则、豁免、历史周报、复盘、成员、商品、店铺和价格记录，不保存登录令牌摘要。保留最近两份内容不同的 JSON；重复刷新不挤掉上一版，服务器版本倒退、请求失败或存储空间不足时保留原备份，并在备份页显示错误。备份时间表示该数据版本的导出时间。

“设置 → 导出与恢复 → 本机完整备份”可断网导出当前或上一版 JSON，菜价页面也可读取完整备份中的数据。退出家庭、登录失效后备份仍保留，可从登录页导出；卸载或清除 App 数据仍会删除备份，需定期导出到其他可信位置。旧版家庭视图会另存为抢救快照，可在登录页导出，但不是可直接导入的完整备份。完整备份不会自动上传并覆盖服务器；只有创建者手动选择文件、确认恢复后才写回当前家庭。如果服务器家庭文件已全部丢失，需要在服务器端用备份重建家庭和登录凭证，现有恢复接口不能恢复到空服务器。

本地验证：`cd mobile && npm test && npm run build`。缓存测试覆盖全量内容、凭证过滤、服务器/家庭隔离、上一版保留、版本倒退、空间不足、旧快照抢救、离线读取和并发同步。
