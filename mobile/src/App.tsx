...
const rewardRuleVersions = {
  v3: { personal: personalRewardTiers, family: familyRewardTiers },
} as const;
  ...
  return (
    <div className="app-shell">
      ...
      <nav className="bottom-nav">
        <NavButton active={tab === "home"} label="今天" icon="☾" onClick={() => setTab("home")} />
        <NavButton active={tab === "records"} label="记录" icon="✓" badge={reviewCount} onClick={() => setTab("records")} />
        <NavButton active={tab === "archives"} label="周报" icon="▥" onClick={() => setTab("archives")} />
...
function Home({ family, loading, onCheckIn }: { family: Family; loading: boolean; onCheckIn: () => void }) {
  ...
  const personalMaximum = weeklyMaximum(family.activeWeek.settings);
  ...
  return (
    <div className="page-stack">
      ...
      <section className="stats-grid">
        ...
        <Stat label="打卡天数" value={String(mySummary.checkinDays)} suffix="/ 7" />
...
function WeeklyReports({ family }: { family: Family }) {
  ...
  const previousWeek = archives.find((archive) => archive.weekStart === addDateDays(family.activeWeek.weekStart, -7));
  return (
    <div className="page-stack">
  ...
  return (
    <section className="week-comparison">
      ...
      <div className="week-comparison-grid">
        <ComparisonStat label="双人积分" value={`${current.score > 0 ? "+" : ""}${current.score} 分`} detail={differenceText(scoreDifference, "分")} positive={scoreDifference >= 0} />
...
function HistoryWeek({ archive, members, expanded, onToggle }: { archive: Archive; members: Member[]; expanded: boolean; onToggle: () => void }) {
  ...
  const level = scoreLevel(familyScore, weeklyMaximum(archive.settingsSnapshot) * members.length);
  return (
    <article className={`history-week ${expanded ? "expanded" : ""}`}>
      <button className="history-week-toggle" onClick={onToggle} aria-expanded={expanded}>
        <div className="history-week-date"><span>{archive.weekStart.slice(5)} — {archive.weekEnd.slice(5)}</span><small>{members.map((member) => `${member.name} ${archive.summary.members[member.id]?.totalScore ?? 0} 分`).join(" · ")}</small></div>
        <div className="history-week-numbers"><span><small>双人积分</small><b>{familyScore > 0 ? "+" : ""}{familyScore}</b></span><span><small>完成度</small><b>{archive.summary.completionRate}%</b></span><span><small>奖励参考</small><b>{reward.total} 元</b></span></div>
...
function WeeklyCard({ title, weekStart, weekEnd, rewardRuleVersion = "v3", days, summary, settings, members, current = false }: { title: string; weekStart: string; weekEnd: string; rewardRuleVersion?: string; days: DayResult[]; summary: WeekSummary; settings: Settings; members: Member[]; current?: boolean }) {
  ...
  return (
    <section className={`card weekly-card ${current ? "current" : ""}`}>
      ...
      <div className={`family-score grade-${familyLevel.tone}`}><span>双人总分</span><strong>{familyScore > 0 ? "+" : ""}{familyScore}</strong><em>分 · {familyLevel.name}</em></div>
      ...
      <div className="weekly-reward-result">
        ...
        <div className="reward-result-rows">
          {reward.personal.map((item) => <div key={item.member.id}><span>{item.member.name}的个人奖励<small>{item.payer ? `由 ${item.payer.name} 转入` : "等待搭档加入"}</small></span><b>{item.amount} 元</b></div>)}
          <div><span>双人累计奖励<small>{reward.familyEligible ? `合计 ${reward.familyScore} 分 · 双方各承担 ${reward.family / 2} 元` : "需两人都至少达到 10 分"}</small></span><b>{reward.family} 元</b></div>
        </div>
...
function SettingsView({ family, backendURL, joinCode, loading, onSave, onCompleteReview, onExport, onRestore, onExit }: { family: Family; backendURL: string; joinCode: string; loading: boolean; onSave: (settings: Settings) => void; onCompleteReview: () => void; onExport: () => Promise<void>; onRestore: (backup: FamilyBackup) => void; onExit: () => void }) {
  const [draft, setDraft] = useState<Settings>(() => structuredClone(family.activeWeek.settings));
  const [draftError, setDraftError] = useState("");
  const [section, setSection] = useState<"root" | "score" | "reward" | "levels" | "review" | "backup">("root");
  ...
  if (section === "reward") {
    return <RewardRulesView onBack={() => setSection("root")} />;
  }

  if (section === "levels") {
    return <LevelRulesView settings={family.activeWeek.settings} memberCount={family.members.length} onBack={() => setSection("root")} />;
  }

  if (section === "review") {
    return <ReviewView family={family} loading={loading} owner={owner} onCompleteReview={onCompleteReview} onBack={() => setSection("root")} />;
  }

  if (section === "backup") {
    return <DataBackupView family={family} owner={owner} loading={loading} onExport={onExport} onRestore={onRestore} onBack={() => setSection("root")} />;
  }
...
function DataBackupView({ family, owner, loading, onExport, onRestore, onBack }: { family: Family; owner: boolean; loading: boolean; onExport: () => Promise<void>; onRestore: (backup: FamilyBackup) => void; onBack: () => void }) {
  ...
  return <div className="page-stack"><SettingsBack eyebrow="DATA BACKUP" title="导出与恢复" onBack={onBack} /><section className="card backup-card"><div className="backup-block"><span className="backup-icon">⇩</span><div><h3>导出家庭备份</h3><p>包含计分规则、打卡、豁免、周报和复盘周期，不包含可直接登录的成员令牌。</p></div><button className="primary" disabled={loading} onClick={() => void onExport()}>导出 JSON</button></div><div className="backup-warning">备份包含手机号和家庭记录，请存放在可信位置，不要发送给无关人员。</div></section><section className="card backup-card"><div className="backup-block"><span className="backup-icon restore">↺</span><div><h3>从备份恢复</h3><p>{owner ? "仅支持恢复同一个家庭、相同成员组成的备份；当前登录凭证会保留。" : "只有家庭创建者可以执行恢复。"}</p></div>{owner && <label className="restore-file"><input type="file" accept="application/json,.json" disabled={loading} onChange={(event) => void selectBackup(event)} /><span>选择备份文件</span></label>}</div>{backupError && <div className="inline-error">{backupError}</div>}<small>当前家庭：{family.name} · {family.id}</small></section></div>;
}
      ...
      <section className="review-member-list">
        {metrics.members.map((metric, index) => (
          <article className="card review-member" key={metric.member.id}>
            <div className="review-member-head"><i style={{ background: memberColor(index) }} /><div><h3>{metric.member.name}</h3><span>{metric.validDays}/{metrics.elapsedDays} 天有效记录 · 完成度 {metric.completionRate}%</span></div><strong>{metric.totalScore > 0 ? "+" : ""}{metric.totalScore} 分</strong></div>
...
function RewardRulesView({ onBack }: { onBack: () => void }) {
  return <div className="page-stack"><SettingsBack eyebrow="BUILT-IN REWARD" title="奖励计算规则" onBack={onBack} /><section className="card reward-rules-card"><p className="rule-intro">每周奖励由“个人奖励”和“双人累计奖励”组成，合计最高 <b>300 元</b>。</p><RuleTable title="个人周奖励" caption="个人奖励由对方转入共同账户，每人最高 100 元。" firstLabel="个人积分" rows={personalRewardTiers} zeroLabel="9 分及以下" /><RuleTable title="双人累计奖励" caption="需两人都至少获得 10 分；奖励由双方各承担 50%。" firstLabel="双人合计" rows={familyRewardTiers} zeroLabel="任一人低于 10 分" /><div className="payment-rules"><div><b>个人奖励怎么付</b><span>A 的奖励由 B 付，B 的奖励由 A 付，都转入共同账户。</span></div><div><b>罚金不变</b><span>一人熬夜时罚金给对方；两人同时熬夜时各自转入共同账户。</span></div><div><b>300 元的口径</b><span>仅限积分奖励，不包含罚金；奖励与罚金不相互抵消。</span></div></div></section></div>;
}
...
function SettingsBack({ eyebrow, title, onBack }: { eyebrow: string; title: string; onBack: () => void }) {
  return <div className="subpage-head"><button onClick={onBack} aria-label="返回设置">‹</button><div><span className="eyebrow">{eyebrow}</span><h2>{title}</h2></div></div>;
}
  ...
  return (
    <div className="tier-editor"><h3>{title}</h3><div className="tier-head"><span>截至</span><span>积分</span><span>罚金</span></div>{tiers.map((tier, index) => (
      <div className="tier-row" key={index}>
        ...
        <input key={`score-${tier.score}`} type="number" inputMode="numeric" disabled={disabled} defaultValue={tier.score} onBlur={(event) => commitNumber(event, index, "score", tier.score)} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} />
...
function ScorePanel({ label, score, level, icon }: { label: string; score: number; level: ScoreLevel; icon: string }) {
  return <div className={`score-panel grade-${level.tone}`}><span>{icon} {label}</span><div><strong>{score > 0 ? "+" : ""}{score}</strong><em>分</em></div><small>{level.name} · {level.description}</small></div>;
}

function ScoreTag({ score, level }: { score: number; level: ScoreLevel }) {
  return <span className={`score-tag grade-${level.tone}`}><b>{score > 0 ? "+" : ""}{score}</b><small>{level.name}</small></span>;
}
...
function weeklyReward(summary: WeekSummary, members: Member[], rewardRuleVersion = "v3") {
  const rules = rewardRuleVersions[rewardRuleVersion as keyof typeof rewardRuleVersions] ?? rewardRuleVersions.v3;
  ...
  const personal = members.map((member, index) => ({
    ...
    amount: tierReward(scores[index], rules.personal),
  }));
  const familyScore = scores.reduce((total, score) => total + score, 0);
  const familyEligible = members.length === 2 && scores.every((score) => score >= 10);
  ...
  return { personal, familyScore, familyEligible, family, total };
}

function weeklyMaximum(settings: Settings) {
...
function comparisonMetrics(days: DayResult[], members: Member[]) {
  ...
  return {
    ...
    score: records.reduce((total, record) => total + record.score, 0),
